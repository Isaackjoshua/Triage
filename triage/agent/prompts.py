"""The runtime system prompt — the `system` content sent on every turn of the agent loop.

This is the text from `triage_agent_system_prompt.md`, verbatim below the rule, with the
interpolated fields filled in at session start from the bound transport, capability level,
target descriptor, and the operator's authorization record.

The safety and epistemic sections are load-bearing, not boilerplate: the code enforces the
read/write split, but only the prompt can make the model *reason* like something operating
on a machine with no healthy backup — biasing toward one more piece of evidence over one
more guess, and toward saying "I don't know" instead of inventing a confident story.

One section is appended that is not in the source document: the read surface, generated
from the command catalog, so what the model believes it can run and what the gate will
actually permit are the same list.
"""
...
## The read surface available to you

`run_read_command` is backed by a classified command catalog. These are the binaries the gate
will permit on the read path; anything else — and any redirection, chaining, or command
substitution — is refused and must go through `propose_remediation` instead. Flags that mutate
state (`smartctl -t`, `dmesg -C`, `journalctl --vacuum-*`) are refused even on a listed binary,
as are flags that would never return (`-f`/`--follow`).
...
def build_system_prompt(
    *,
    ...
    """Fill the prompt in for one session."""
    ...
    return SYSTEM_PROMPT.format(
        ...
        read_surface=catalog.describe_read_surface(),
    )
